# fish/conf.d/tide-config.fish
#
# tide プロンプトの設定を universal 変数に投入するスクリプト。
# 新マシンで初回 fish 起動時に1回だけ実行される。
#
# 仕組み:
#   - tide_dotfiles_applied マーカーがあればスキップ
#   - なければ全 tide_* 変数を set -U で投入し、マーカーをセット
#
# 上書きしたい場合: `set -e tide_dotfiles_applied` で再適用
# 手動 `tide configure` の結果はマーカー有りなので保護される

if not status is-interactive
    exit
end

if set -q tide_dotfiles_applied
    exit
end
...
set -U tide_gcloud_display_strategy never
...
set -U tide_kubectl_display_strategy never
...
set -U tide_dotfiles_applied yes
