...
## Language
- ユーザーへの応答は常に **日本語** で行う。
- 説明・進捗報告・エラーメッセージなど、ユーザー向けの出力はすべて日本語で書く。
- 技術用語やコード識別子（関数名・変数名・コマンド等）は原文のままにする。
- ダイアクリティカルマークやアクセント（例: "não" を "nao" としない、"für" を "fur" としない）は省略せず正しく表記する。

## Asking the user
- 不明点・前提条件が曖昧な点・複数の妥当な選択肢がある場合は、推測で進めずにユーザーに確認する。
- 質問は短く、判断材料（メリット・デメリット、影響範囲、推奨理由）を添えて選択肢を提示する。
- 1回あたり最大4問にまとめる。確認漏れがあれば次のターンで再度質問する。
