---
name: spec-audit
description: 仕様書と実装の乖離を全方位チェックし、未実装・TODO・スキップテスト・APIスキーマ乖離を検出して Issue 化する。ユーザーが仕様 vs 実装の監査、漏れ検出、未実装機能棚卸しを依頼したときに使う。
---
...
2. サブエージェントで検索・調査するか、grep / glob で対応する実装を探す
3. 一致・不一致・未実装を判定
