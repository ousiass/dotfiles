---
name: stitch-shopify
description: Stitch で生成したモック zip を、Shopify Online Store 2.0 のテーマディレクトリ（Atomic Design ベースの snippets 構造、`section.settings` 化テキスト、`image_picker` 化画像）に変換する。ユーザーが「Stitch モックを Shopify テーマにしたい」「OS 2.0 テーマを Stitch から起こしたい」と依頼したときに使う。
---
...
Shopify標準9種（`index`, `product`, `collection`, `cart`, `page`, `blog`, `article`, `404`, `search`）のうちモックに無いものを列挙し、**各テンプレートごとに選択式でユーザーに対応を選ばせる（自由入力を許容）**：
