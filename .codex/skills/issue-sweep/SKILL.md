---
name: issue-sweep
description: 複数のオープン Issue をキュー化し、Stop Hook と連動して端から自律的に実装・PR マージまで進める。複数 Issue を一括で消化したいとき、またはフェーズ Issue 配下の子 Issue 群をまとめて実装したいときに使う。
---

# issue-sweep

複数の GitHub Issue を端から自律的に連続実装するスキル。`.sweep/queue.txt` にキューを書き出し、Stop Hook (`hooks/check-issue-queue.sh`) と連動してキューが空になるまで停止できないようにする。
...
- `/issue-sweep --parallel <N>` — 同時に処理する Issue 数（**デフォルト 5**、上限 5）。依存関係のない Issue を最大 N 件並列でサブエージェントに渡す。ユーザーに値を確認せず常にこのデフォルトで起動する
...
3. **Issue の自動分割（fan-out）**: 以下の除外条件に該当しない全 Issue について、サブエージェントを起動して `/issue-split-auto #<n>` を実行させる。**文字数や H2 数のような表層メトリクスで事前フィルタしない** — 短くてもスコープが混在してることはあるし、長くても単一機能で分割不要なことはある。split-auto 側で本文と関連仕様書を実際に読んで判定させる:
   - `bug` ラベルが付いていない
...
**重要 — context 設計:**
**1 Issue 分の実装（Plan→Develop→Review→Commit→Push→PR 作成）は必ずサブエージェントに丸投げする。** メインスレッドは「キュー操作 / 冪等性チェック / サブエージェント起動 / CI 緑ポーリング / 直接マージ / 失敗判定」だけを行う。これによりメイン context は Issue 数に対して線形に汚れず、PR URL の一覧だけが積まれる。
...
- N 件のサブエージェントを **同一メッセージ内で並列起動**（同時に N 個立ち上げる）
- 全サブエージェントの返答を集めた後、各 PR を順にポーリング（2-4）→ Issue close（2-5）→ キューから該当行を削除（2-6）
...
- `state == OPEN` → **既存 PR あり**。サブエージェント起動（2-3）はスキップし、PR 番号を引き継いで 2-4（ポーリング）へ
- それ以外 → 通常フロー（2-3）

### 2-3. サブエージェントで Issue を1件丸ごと処理

**worktree スナップショット**: サブエージェント起動前に `git worktree list --porcelain | grep '^worktree ' | awk '{print $2}' | sort > /tmp/wt-before` を実行（2-7 の差分検知で使用）。

サブエージェントを以下の指定で起動する:

- 役割: catch-all（全ツール利用可能な汎用エンジニア）
- 用途説明: `"Issue #<n> implementation"`
- プロンプト: 自己完結したプロンプトを渡す。**初回起動**と **CI fix 起動** の2モード:
...
1. `gh issue view <n> --json labels` でラベルを取得し、以下のマッピングでスキル選択:
   ...
   - design → /design-fix #<n>
   - それ以外 → /impl-wt #<n>
2. 選択したスキルを呼び出し、Plan→Develop→Review→Commit→Push→PR 作成まで完了させる。
   各サブスキルの禁止行動（フェーズスキップ・テスト省略・サイレントスキップ・スコープ外発見の未 issue 化）は厳守。
3. **PR 作成後、続けて `/refine --no-merge` を呼び出し、4 観点（code-review / doc-drift / spec-audit、HALT 検知時は halt-review）で並列レビューして critical/major=0 ∧ minor≤5 まで研磨させる。マージは行わせない（--no-merge）**。refine の最終結果から `refine_status` / `critical_remaining` / `major_remaining` / `minor_remaining` を取得する。
4. **マージゲート判定**（必須）:
...
- マージはメインスレッドが行うので、サブエージェントは PR 作成までで返す（`gh pr merge` は叩かない）。
...
サブエージェントの返答 JSON を parse して PR 番号を取得する。`failure` が返ったら 2-7（失敗時挙動）へ。
...
while true; do
  ...
  if [[ "$state" == "CLOSED" && "$merged" == "null" ]]; then
    # 手動 close 検知 → 1回目はサブエージェント再起動、2回目はユーザー判断
  ...
  if [[ -n "$failed_checks" && "$pending" -eq 0 ]]; then
    # CI 確定失敗 → CI fix 起動プロンプトでサブエージェント再 spawn
    ...
    respawn_count=$((respawn_count+1))
    gh pr comment <PR> --body "sweep: CI 失敗を検知（attempt ${respawn_count}/3、checks: $failed_checks）。修正エージェントを再起動します。"
    sweep_notify "CI failed" "PR #${PR} attempt ${respawn_count}/3: $failed_checks" ":warning:"
    # 2-3 の「CI fix 起動プロンプト」を使ってサブエージェント起動。返答 fixed=true なら continue で次ループへ
    continue
  fi
...
- **respawn 上限**: 同一 PR でサブエージェント再起動を **2回まで**。3回目で `gh run view` ログを添えてユーザーに判断を仰ぐ
- **CLOSED null**（手動 close）: 1回目はサブエージェント再起動、2回目で諦める
...
サブエージェントが成功/失敗どちらでも、その反復で作られた worktree が残っていれば掃除する:

```bash
# サブエージェント起動前後の worktree 差分から「この反復で作られた」 path を特定
...
# サブエージェントが failure を返したとき: 作成した worktree を強制削除
# サブエージェントが success を返し PR がマージ完了したとき: branch ごと削除（--delete-branch で remote 側は処理済み、local worktree のみ掃除）
for wt in $new_paths; do
  git worktree remove --force "$wt" 2>/dev/null || true
done
```

サブエージェントが `failure` を返した場合は同じ Issue で次回再起動時に worktree 衝突しないよう **必ず** 削除する。

### 2-8. 次の反復 / 失敗時
- 正常完了: 2-1 に戻る。停止しようとしても Stop Hook が押し戻す
- サブエージェントが `failure` を返した:
...
- **メインスレッドで直接サブスキル（`/impl-wt` 等）を起動する**（context 汚染の根本原因。必ずサブエージェント経由）
- **メインスレッド自身がコードを修正する / コミットする / PR を編集する**（CTO は実装に手を出さない。修正は必ず CI fix 起動プロンプトでサブエージェントに委譲）
- **engineer エージェント内で `/refine --no-merge` をスキップする**（4 観点レビューを通さずマージに進むと品質ばらつきが出る）
...
- **メインが CI 緑後にマージするのを忘れる**（サブエージェントは PR 作成までで返るため、メインがポーリングして直接マージしないと sweep が永久に止まる）
- **サブエージェント内で `gh pr merge` を叩く**（マージはメインの責務。サブエージェントは PR 作成までで返す）
- マージ完了確認をスキップして次の Issue に進む（PR が closed/CI fail なまま埋もれる）
- **CI 失敗を検知せずポーリングを継続する**（無限待機の原因）
- **失敗 check 名をサブエージェントに伝えず「とりあえず再実行」を頼む**（サブエージェントが原因不明のまま盲目的に手を入れる事故を防ぐ）
- サブエージェントの返答 JSON 以外をメイン context に取り込もうとする（サブエージェント内部の Plan/Develop/Review ログをメインに残すのは禁止）
