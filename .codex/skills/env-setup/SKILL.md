---
name: env-setup
description: 既存仕様書に環境構築セクション（CI/CD、Lint、Format、Linterly、Git Hooks、Swagger/OpenAPI 等）を対話的に追加する。spec-gen 等で作成済みの仕様書を持つプロジェクトに、開発環境とツールチェインのドキュメントを揃えたいときに使う。
---
...
- パッケージマネージャ
  ...
  - 既存プロジェクトに `package-lock.json` / `yarn.lock` / `pnpm-lock.yaml` がある場合は、bun への移行をユーザーに提案する（既存ロックファイルを残すか、`bun.lockb` / `bun.lock` に切り替えるかを選択式で確認する）
- 既存ツールの有無

## フェーズ2: ヒアリングと構成決定

各セクションについて推奨構成を提示し、選択式でユーザーに確認する（自由入力を許容）。
不明点はまとめて聞く（1回4問以内）。
...
2. ユーザーにレビュー提示（選択式で確認、「問題なし」選択肢を含め、自由入力を許容）
