---
name: spec-to-hugo
description: 既存の `spec/` や `docs/` 配下にある Markdown 仕様書を、Hextra テーマを使った Hugo ドキュメントサイトとして公開できる構造に変換する。ユーザーが「仕様書を Hugo サイトにしたい」「Hextra で社内ドキュメントを公開したい」「Markdown 仕様書を Netlify/Cloudflare Workers で配信できる形にしたい」と依頼したときに使う。
---
