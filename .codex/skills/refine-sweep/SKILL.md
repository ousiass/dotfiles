...
各レビュースキルを **Issue 化モードで並列起動**。サブエージェントは Skill を呼び出し、Skill 内でユーザーに出力先を選ばせる質問（例: `GitHub Issue に作成` / `個別 Issue`）があれば自動的に GitHub Issue 側を選び、承認確認も自動で先へ進む。

```
[サブエージェント1] description: "refine-sweep iter <iter+1> — code-review"
プロンプト:
リポジトリ全体に対して /code-review を起動して実行してください。

Issue 化方針（厳守）:
- 検出した指摘は GitHub Issue に作成する（review スキル本来の挙動を活用）
- スキル内で出力先の選択を求められた場合は「GitHub Issue に作成」を選択
- Issue の作成方式を求められた場合は「個別 Issue」を選択（1 指摘 = 1 Issue）
- 承認確認を求められた場合は承認して先に進む
- ユーザーへの追加確認で停止しないこと
...
if [[ $(echo "$new_issues" | jq '.failures | length') -gt 0 ]]; then
  # review サブエージェントが一つでも failure → agent_failed でフェーズ3 へ
  status="agent_failed"
fi
```
...
label=`refine-sweep` の全 open Issue を **`/impl-wt` で消化 → PR → CI 緑 → merge → Issue close** まで完結させる。メインスレッドは Issue キューを組み、engineer サブエージェントを wave 分割で並列起動して JSON だけ受け取る。
...
各 Issue の engineer サブエージェントへのプロンプト:
...
```bash
# 各 wave で並列起動したサブエージェントの JSON を集約
...
- **メインスレッドで PR マージ完了のポーリングや CI fix ループを直接回す**（impl-wt サブエージェント内に閉じ込める）
- **review サブエージェントに Issue 作成を止めさせる**（旧設計。現在は Issue 化が意図的設計）
- **fix サブエージェントが `impl-wt` スキルを呼ばず自前で PR を作る**（impl-wt に一任することでフローを統一）
- **fix サブエージェントが `gh pr merge --auto` を使う**（CI 緑ポーリング → 直接 `--merge` に統一）
- **fix サブエージェントが Issue を close せずに PR merge で済ませる**（`Closes #N` 記法 or 明示的な `gh issue close` で必ず close）
...
- review サブエージェント failure（1 つでも `failure != null` を返す）: `agent_failed` を記録しフェーズ3 へ
- impl-wt サブエージェント failure が 2 反復連続で同じ Issue: `refine-sweep-stuck` ラベルを付与し次反復から除外して継続
