---
name: refine-sweep
description: 全コードベースを 4 観点で連続レビューし、ドメイン別並列 PR で critical/major=0 ∧ minor≤5 まで磨く。残 open Issue も自動的に impl-wt に委譲して実装まで完了する。リポジトリ全体の品質をゼロ近くへ押し下げたいときに使う。
---
...
`/refine` の 2-1 と同じ形式でサブエージェントを並列起動する。ただし**対象は PR ではなく base ブランチの HEAD**:

```
[サブエージェント1] description: "refine-sweep iter <iter+1> — code-review"
プロンプト:
リポジトリ全体に対して /code-review を起動して実行。
...
### 2-5. ドメイン別並列 fix engineer サブエージェント（メインは JSON だけ受け取る）

**CTO 原則**: メインスレッドはコード修正・コミット・PR 操作・CI 待ち、いずれにも直接タッチしない。各ドメインの fix を **engineer サブエージェントごとに 1 つ立て、同一メッセージで並列起動**する（issue-sweep の `--parallel` と同じ発想）。
...
- **指摘が 0 件のドメインはサブエージェント起動せずスキップ**

各ウェーブ内の各ドメイン fix サブエージェントへのプロンプト:
...
**範囲外のファイルは絶対に変更しない**（他ドメインのサブエージェントと競合するため）。範囲外の修正が必要と判断したら failure で返す。
...
3. 実装サブエージェントで順に修正＋テスト（範囲外のファイルを触らない厳守）
...
refine 終了時点で **open のままになっている全 Issue** を、refine-sweep のメインスレッドが直接キュー化し、各 Issue をサブエージェント + `/impl-wt` で 1 件ずつ実装する。**`/issue-sweep` はネスト呼びしない**（`.sweep/state.json` を奪い合うため、進捗追跡不能・勝手に terminal 化される問題を回避）。

sweep 中に develop エージェントが `/spinoff-issue` で作った新規 Issue だけでなく、事前から残っていた Issue も含めて処理対象に入れる（refine で `## 派生 issue` をうまく検知できなかった / ラベルが付かなかった等の検出漏れに対する保険）。
...
- 1 件以上あり、`--no-follow-spinoffs` 指定なし、`round < max_rounds`:
  ...
  - **各 Issue を sequential にサブエージェント + `/impl-wt` で実装**:

    ```
    for issue_num in $remaining_ids; do
      echo "$PPID:$(date +%s)" > .sweep/lock   # 各 Issue 処理前に heartbeat

      [サブエージェント] description: "refine-sweep round <round>: impl-wt #${issue_num}"
      プロンプト:
  ...
  - 各サブエージェントの JSON を集めて round 結果を記録:
...
- **メインスレッドで PR マージ完了のポーリングや CI fix ループを直接回す**（コンテキスト線形保持のため fix engineer サブエージェント内に閉じ込める。issue-sweep のフェーズ2-4 のようにメインで sleep するのではなく、refine-sweep では engineer サブエージェントが内部で待つ）
- **fix サブエージェントの Plan/Develop/Review/Push log をメイン context に取り込もうとする**（JSON 1行のみ受け取る）
- review サブエージェントと fix サブエージェントを同じ呼び出しで混ぜる
- **ドメイン分けせず 1 PR に全 critical+major 修正を詰める**（差分肥大化・レビュー困難・競合多発の元）
- **fix サブエージェントがドメインのファイル範囲を超えて他ドメインの src を編集する**（scope_violation で返すべき）
...
- fix サブエージェントが `--no-merge` で済ませる（CI 緑後に `gh pr merge --merge --delete-branch` を必ず実行）
- **fix サブエージェントが `gh pr merge --auto` を使う**（リポジトリ設定 `allow_auto_merge` に挙動が依存し、OFF だと GraphQL エラーで止まる。CI 緑をポーリングしてから直接マージする方式に統一）
- ユーザーに「続けますか」と聞く（Stop Hook が押し戻す）

## 失敗時の挙動

- review サブエージェント failure: テレメトリに `agent_failed` を記録しレポート生成して終了
- fix サブエージェント failure: 反復ブランチを削除（`git push origin --delete <branch>`、可能なら）してテレメトリ記録・レポート生成・終了
