---
name: doc-drift
description: ドキュメントと実装コードの整合性をチェックし、乖離レポートを生成する。ユーザーが仕様書と実装のズレ確認・ドキュメント乖離調査・Issue 化を依頼したときに使う。
---
...
2. サブエージェントで検索・調査し、`Grep`/`Glob` で対応する実装コードを探す
