---
name: stitch-deploy
description: Stitch で生成したモック zip を展開し、Netlify にそのままアップロードできるディレクトリ構造（ページごとに `index.html` + `screen.png`、ナビゲーション付きトップページ）に変換する。ユーザーが「Stitch のモックを Netlify で公開したい」「Stitch zip を Web で見られる形にしたい」と依頼したときに使う。
---
