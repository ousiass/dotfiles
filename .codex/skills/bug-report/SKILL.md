---
name: bug-report
description: 対話形式でバグをヒアリングし、コード調査の上で構造化されたGitHub Issueを作成する。ユーザーがバグ報告・Issue 化を依頼したとき、特に非エンジニアが症状だけ伝えてきたときに使う。
---
...
- **テキスト** (例: `ログインできない`): 初期情報として扱い、不足分をヒアリング
- **引数なし**: 最初からヒアリングを開始
...
選択式でユーザーに以下をまとめて確認する（自由入力を許容。引数でわかっている情報はスキップ）：
...
1. **関連コードの特定** — 画面名・機能名・エラーメッセージで `Grep`/`Glob` 検索。必要に応じてサブエージェントで検索・調査して広範に確認
