---
name: html-shopify
description: 任意のHTMLモック（zip/ディレクトリ/単一ファイル）をShopify OS 2.0テーマ（Atomic Design snippets構造）に変換する。ユーザーが Figma export / Webflow export / 手書きHTML / その他ツール出力の HTML を Shopify テーマ化したいとき、`shopify theme dev` で動作するテーマを生成したいときに使う。
---
...
1. **入力タイプ判定**
   - 引数未指定 → 作業ディレクトリ内の `*.zip` / `*.html` / HTMLを含むサブディレクトリを探索し、候補を選択式でユーザーに確認（自由入力を許容）
   - 引数が `.zip` → 一時ディレクトリに展開
   - 引数がディレクトリ → そのまま使用
...
選択式でユーザーに確認する（自由入力を許容）。ページ数が多い場合は以下の方針でバッチング：
...
Shopify標準9種のうちマッピングされなかったものを列挙し、**選択式で各テンプレートごとに対応を選ばせる**（自由入力を許容）：
...
1. **リポジトリ判定**: 出力先から `git rev-parse --show-toplevel` を実行
   ...
   - **gitリポジトリ外の場合**: 選択式でユーザーに確認する（自由入力を許容）
...
5. **現在ブランチがmain/masterの場合は警告**: 新規ブランチ作成を推奨し、選択式でユーザーに確認する（自由入力を許容）
