---
name: refine
description: review→修正→再 review を反復し、PR を critical/major=0 ∧ minor≤閾値 まで磨いた後、CI 緑を待って直接マージし Issue を close する。特定 PR の最終仕上げや、impl 系スキルが作った PR の品質ゲートとして使う。
---
...
3. **worktree 確保（必須）**: メイン作業ツリーを保護するため、以下のいずれかで refine 用の worktree を確保する:
   - **既に worktree 内で起動された場合**（例: issue-sweep の engineer エージェントからの呼び出し）: `git rev-parse --show-toplevel` と `git worktree list --porcelain` を比較し、現在が worktree なら**再利用**（新規作成しない）
...
各反復で**レビュースキルごとにサブエージェントを並列起動**する。**メインスレッドはコードに触れない**。

### 2-1. レビュー集約（並列）

決定したスキル群を**同一メッセージで並列に**起動する。各サブエージェントは専門スキルを 1 つだけ実行し、JSON で指摘を返す。

```
[サブエージェント1] description: "Refine iter <iter+1> — code-review"
プロンプト:
PR #<n>（branch: <branch>）に対して /code-review を起動して実行。
...
[サブエージェント2] description: "Refine iter <iter+1> — doc-drift"
同様、/doc-drift を実行、 "source": "doc-drift" で返す

[サブエージェント3] description: "Refine iter <iter+1> — spec-audit"
...
while true; do
  ...
  if [[ -n "$failed" && "$pending" -eq 0 ]]; then
    ...
    respawn=$((respawn+1))
    gh pr comment <PR> --body "refine: マージ前 CI 失敗を検知（attempt ${respawn}/3、checks: $failed）。修正エージェントを再起動します。"
    # 2-3 と同じ CI fix プロンプトでサブエージェント起動
    continue
  fi
...
6. **呼び出し元への返答**: 最終メッセージとして以下の JSON 1行を出力（issue-sweep の engineer エージェントなどが parse できるよう、Markdown レポートのパス案内に**先行して** JSON 行を出すこと）:
...
- review サブエージェントと fix サブエージェントを同じ呼び出しで混ぜる（独立性を保つ）
...
- review サブエージェント / fix サブエージェントのいずれかが failure → ループ中断、テレメトリ最終行に `status: "agent_failed"` を記録、レポート生成して終了
