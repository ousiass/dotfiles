...
- `--auto`: **自律実行モード**。`issue-sweep` 等から呼ばれる場合は必ず付ける。**ユーザーが応答できない前提**で動く:
  - 他の assignee がいても警告 1 行で続行する（着手可否を聞いて止まらない）
  - 症状・再現手順などの不足情報はユーザーに聞かず、Issue 本文・コード・ログから**最も妥当な再現条件を推定**し、採用した前提を PR 本文の「## 前提」に列挙する
  - 推定した前提では原因を特定できない場合は、推測で修正せず failure として返す（間違った修正を入れるより返す方が安全）
  - **回帰テストの追加は省略禁止**（`--auto` でも必須。むしろ確認を省くぶんテストが唯一の担保になる）
