...
- `--auto`: **自律実行モード**。`issue-sweep` 等から呼ばれる場合は必ず付ける。**ユーザーが応答できない前提**で動く:
  - 修正内容の不明点はユーザーに聞かず、Issue / レポートの記載とデザイントークンの定義から最も妥当な解釈を採用し、採用した前提を PR 本文の「## 前提」に列挙する
  - `review` エージェントによるレビューをスキップする（後段の `/refine-git` が同じ差分を多観点でレビューするため二重コストになる）。**デザイン観点の検証（トークン準拠・アクセシビリティ）は省略禁止** — これは refine-git のレビュー観点に含まれない
...
1. 全指摘の修正完了を確認
2. `gh pr create --base <ベースブランチ>` でPRを作成（**`--no-pr` 指定時はスキップ**して commit + push までで返し、最終報告に「PR 未作成（呼び出し元に委譲）」と明記する）
