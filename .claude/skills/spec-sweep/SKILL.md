...
`AskUserQuestion` で運用方針を確認（4 問以内/回）:
...
続けて、項目ごとに以下を `AskUserQuestion` で収集（項目数だけ繰り返す）:
...
ヒアリングは項目ごとに 1 回の `AskUserQuestion` にまとめる。

### 1-2: 計画確定

収集情報を整理して提示し、`AskUserQuestion` で承認を取る:
...
承認後、`TaskCreate` で各項目を 1 タスクとしてキュー化する。
...
- 計画フェーズで集めた情報を活用し、執筆中の `AskUserQuestion` は極力控える
