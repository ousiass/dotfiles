...
実装中に**現スコープと独立した発見**（無関係なバグ・スコープ外の改善・別途必要なリファクタ等）があれば、TODO 残しにせず `/spinoff-issue --parent <親Issue番号> <要約>` で別 issue 化してから現スコープを継続する。フェーズ1で取得した親 Issue 番号は必ず `--parent` で渡し、親なし（テキスト起動）の場合のみ省略可。
