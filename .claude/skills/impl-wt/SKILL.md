...
実装中に**現スコープと独立した発見**（無関係なバグ・スコープ外の改善・別途必要なリファクタ等）があれば、**発見した瞬間に作業を中断し** `/spinoff-issue --parent <親Issue番号> <要約>` を呼んで issue を作成してから現スコープに戻る。サブステップ末尾・スコープ末尾・フェーズ3 にまとめて処理することは認めない（記憶から落ちる／PR 本文の列挙で代替してしまう事故を防ぐため）。フェーズ1で取得した親 Issue 番号は必ず `--parent` で渡し、親なし（テキスト起動）の場合のみ省略可。
...
3. **スコープ外発見のセルフチェック**: 実装中に発見したスコープ外の事項で未 issue 化のものが残っていないかを明示的に確認・宣言する。残っていればこの時点で `/spinoff-issue` を呼んで issue 化してから次に進む。PR 本文や最終報告に「発見だが未 issue 化」を書くのは禁止
