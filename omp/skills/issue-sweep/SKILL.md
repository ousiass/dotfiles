...
**バッチの既定上限は 3 Issue。** `scope` の重なりや同一 parent の兄弟で 4 件以上が同じバッチに集まった場合は、`depends_on` の順序を保ったまま先頭から 3 件ずつに区切り、複数バッチに分ける。依存関係の都合で先頭 3 件だけでは実装できない場合に限り超過を許容し、「#a,#b,… の N 件を依存のため 1 バッチにまとめます（PR が大きくなります）」と 1 行宣言してから進む（確認は取らない）。

**重い unit（CI / fuzz / 長い実装・大規模 rename など、1 本で `task.softRequestBudget` を消費しやすいもの）は上限 1。** `heavy` ラベル、または本文・タイトルに CI・fuzz・大規模 rename 等の記述があれば、`scope` が重なっていても他 unit と混ぜず単独バッチにする。

`--no-batch` 指定時はこの節全体を無効化し、常に 1 Issue = 1 バッチ（= 1 PR）にする。
...
**重要 — context 設計:** **1 バッチ分の実装（Develop→Commit→Push→PR 作成→研磨）は必ず `task` ツールに丸投げする。** メインスレッドは「キュー操作 / PR 一覧の取得 / agent 起動 / 判定 / マージ / 失敗処理」だけを行い、メイン context には PR 番号と状態だけを積む。
...
2-0 で取得した PR 一覧から in-flight の各 PR を引き、上から順に最初に当たったものを実行する。**判定と agent 再起動はここ（LLM 側）で行う。bash の `while` ループの中で `task` ツールは呼べない。**
...
- **bash の `while` ループの中で agent を再起動しようとする**（bash から `task` ツールは呼べない。状態取得と判定を分ける理由がこれ）
...
- **メインスレッドで直接サブスキル（`/impl` 等）を Skill ツール起動する**（context 汚染の根本原因。必ず `task` 経由）
