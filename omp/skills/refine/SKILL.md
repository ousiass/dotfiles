...
各反復でレビュースキルごとに並列サブエージェントを起動する。起動は `task(agent=review)`（モデルは `modelRoles` が解決するので指定しない）。**メインスレッドはコードに触れない**。
...
**2 周目以降はレビュー範囲を絞る。** リポジトリ全体を毎回スキャンし直すのは無駄（1 周目で見たコードの大半は修正で変わっていない）:

```bash
# レビュー範囲 = 前回の修正で触ったファイル ∪ 未解決指摘があるファイル
touched=$(git diff --name-only "$prev_head"...HEAD)   # prev_head は 2-4 の修正 push 前に控えた値
unresolved=$(echo "$findings" | jq -r '(.critical + .major + .minor)[].file // empty')
review_scope=$(printf '%s\n%s\n' "$touched" "$unresolved" | sort -u | grep -v '^$')
```

次の反復では各レビュー agent に「リポジトリ全体」ではなく `review_scope` を渡す。**全体スキャンは 1 周目だけ**でよい。
...
- **2 周目以降もリポジトリ全体をスキャンし直す**（2-5 の `review_scope` に絞る。全体スキャンは 1 周目だけ）
