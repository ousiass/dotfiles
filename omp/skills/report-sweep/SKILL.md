...
項目ごとに 1 回の `ask` にまとめる。バグ・機能要望共通で以下を集める:
...
`ask` で「一括承認 / 修正指示 / 中止」を確認。承認後、`todo` で各項目をタスクキュー化し、フェーズ 3 は割り込みなしで最後まで走らせる（プレビューモードが「各項目で確認」の場合のみ Issue 直前で承認を挟む）。
...
- 類似あり → タイトルと URL を提示し、`ask` で「新規作成」/「既存にコメント追加」/「スキップ」から選ばせる（判断保留になる唯一の例外）
- 類似なし → 続行
